...
const day25 = () => {
    ...
    const move = (situation, callback) => situation.map((row) => row.map((type, x) => // east
        type === '>' && (row.at(x + 1) ?? row.at(0)) === '.' ? callback(type) ?? '.' :
            type === '.' && (row.at(x - 1) ?? row.at(-1)) === '>' ? '>' : type
    )).map((row, y, situation) => row.map((type, x) => // south
        type === 'v' && (situation.at(y + 1) ?? situation.at(0)).at(x) === '.' ? callback(type) ?? '.' :
            type === '.' && (situation.at(y - 1) ?? situation.at(-1)).at(x) === 'v' ? 'v' : type
    ));
    // const display = (situation) => situation.map(row => row.join('')).join('\n');
    const wait = (before, step = 1) => {
        const stats = { changed: false, changes: 0 };
        const callback = () => { // nasty mutation
            stats.changed = true;
            stats.changes += 1;
        };
        const after = move(before, callback);
        return stats.changed ? wait(after, step + 1) : step;
    };
    console.log(wait(raw));
};
day25();